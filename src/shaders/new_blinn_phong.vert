...
void main()
{
	gl_Position = view_matrix * model_matrix * projection_matrix * vec4(_vertex_position, 1.0f);
	fragment_position = vec3(view_matrix * model_matrix * vec4(_vertex_position, 1.0f)); // Transforming vertex position from local space to view space
