...
mat3x3 calculateLight(Light light);
...
void main()
{
	...
	material_diffuse = vec3(1.0f);
	material_specular = vec3(1.0f);
...
vec3 calculateSpotLight(Light light)
{
	...
	mat3x3 light_components = calculateLight(light);
...
vec3 calculateDirectionalLight(Light light)
{
	...
	vec3 this_diffuse  = light.strength * light.color      * material.color * diffuse;
	vec3 this_specular = light.strength * light.specular   * material.color * material.specular_strength * specular;
	vec3 this_ambient  = light.strength * light.ambient    * material.color * environment.ambient_light;
...
std::string phong_vertex_backup_glsl = R"~(
#version 460 core
layout (location = 0) in vec3 _vertex_position;
layout (location = 1) in vec3 _vertex_normal;
layout (location = 2) in vec2 _vertex_texture_coordinate;
...
std::string phong_fragment_backup_glsl = R"~(
#version 460 core
#define MAX_NUMBER_OF_LIGHTS 20
out vec4 FragColor;
